...
Drift’s Builder Code (DBC) system enables any builder to build on top of Drift **while earning fees** for routing trades. Fees are received on a per-fill basis that a builder sends on behalf of a user. 
...
**Per-Order Flexibility**: Builders can attach their code to each individual order they route, with customizable fees.
...
**Builder Registration:** To be eligible for a builder account, Builders are required to hold a minimum asset balance of 1 dSol or 250 Drift.

**User registration:** Users approve builders via the `approve_builder_code` instruction. This stores the builder code within a user's `UserStats` account. Users are therefore required to either have an existing Drift account, or create one on first use of the builder code.

**Fee accrual:** The separately created builder account accrues fees in the following way:
