...
import useSubscribedAssetWeightsData from "../../hooks/useSubscribedAssetWeightsData";
...
const liveWeightData = useSubscribedAssetWeightsData([]);
return <table>
  ...
  <tbody>
    {liveWeightData && liveWeightData.length > 0 ? liveWeightData?.map((row, i) => {
      return (
        <tr key={i} className={row[6] ? modStyles.flashUpdate : ""}>
