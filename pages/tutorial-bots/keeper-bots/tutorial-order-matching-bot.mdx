...
See [Keepers & Decentralised Orderbook](/about-v3/decentralized-orderbook) for a technical explanation of how the decentralised orderbook (DLOB) and matching incentives work.
...
Follow the instructions at [Keeper Bots](/about-v3/keepers) to set the required environment variables and make sure a `ClearingHouseUser` is initialized.
...
Market orders that are sent on the Drift Protocol first go through the [Just-In-Time (JIT) Auctions](/about-v3/matching-engine). After the auction period, Matching Bots step in to fill orders for a small reward.
