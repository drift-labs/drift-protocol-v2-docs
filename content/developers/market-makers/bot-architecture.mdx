...
2. Start OrderSubscriber (websocket + resync)
3. Start PriorityFeeSubscriber
4. Register SIGINT/SIGTERM handlers
5. Main loop:
   a. Check health → emergency cancel if needed
   b. Refresh oracle prices
   c. Update quotes (cancel-and-place or oracle offset)
   d. Process JIT auctions (if participating)
   e. Log metrics (position, PnL, fill rate)
6. Sleep until next cycle
```

See [keeper-bots-v2](https://github.com/drift-labs/keeper-bots-v2) for the reference implementation of these patterns.
