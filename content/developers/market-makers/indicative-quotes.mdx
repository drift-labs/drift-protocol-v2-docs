...
<details>
<summary>View IndicativeQuotesSender import</summary>
...
If you set `isOracleOffset: true`, the `bidPrice` and `askPrice` are interpreted as offsets from the oracle price rather than absolute prices. This is analogous to [oracle offset orders](/developers/market-makers/normal-mm#oracle-offset-orders) — the quote floats with the oracle automatically.
...
To stop publishing quotes for a specific market, send a quote with `null` fields. To stop all quotes across all markets, set `marketIndex` to `null`:

```typescript
// Stop quoting a specific market
quoter.setQuote({
  bidPrice: null,
  askPrice: null,
...
// Stop all quotes across all markets
quoter.setQuote({
  bidPrice: null,
  askPrice: null,
  bidBaseAssetAmount: null,
...
- **Oracle offset mode simplifies maintenance** — using `isOracleOffset: true` means you don't need to update quotes when the oracle moves. Only update when changing spread or size. This mirrors [oracle offset orders](./normal-mm#oracle-offset-orders) for onchain quoting.
...
- [Normal MM](./normal-mm) - Resting onchain orders (committed liquidity)
