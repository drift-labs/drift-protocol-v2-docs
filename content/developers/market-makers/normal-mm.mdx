...
<details>
<summary>View OrderParams import</summary>
...
### Oracle offset orders {#oracle-offset-orders}

Oracle offset orders are the **most efficient way to quote on Drift**. Instead of specifying a fixed price, you set an offset from the oracle price. The order automatically floats with the oracle — when the oracle moves, your order's effective price moves with it.
...
// Atomically cancel all perp orders for market 0 and place new quotes
const { cancelTxSig, placeTxSig } = await driftClient.cancelAndPlaceOrders(
  // Cancel params
  {
    marketType: MarketType.PERP,
    marketIndex: 0,
...
If you're reacting to on-chain taker auctions, Drift exposes a helper that "places a maker order and fills against a taker" atomically. This lets you participate in JIT auctions even while running resting orders — a hybrid approach.
...
- **32-order limit per subaccount** — if you quote 5 markets × 2 sides × 3 levels = 30 orders, you're near the limit. Use multiple subaccounts for multi-market strategies (see `JitMaker` config for subaccount-per-market pattern).
