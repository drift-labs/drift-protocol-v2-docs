...
async function fillSwiftOrder(signedMessage) {
  ...
  const takerIsLong = signedMsgOrderParams.direction === PositionDirection.LONG;
  const makerOrderParams = getLimitOrderParams({
    marketIndex: signedMsgOrderParams.marketIndex,
    direction: takerIsLong ? PositionDirection.SHORT : PositionDirection.LONG,
    baseAssetAmount: signedMsgOrderParams.baseAssetAmount, // fill full size
    price: myFillPrice, // your calculated fill price
  ...
  const takerInfo = {
    ...
    userAccount: takerUserAccount, // from userAccountGetter or userMap
...
**Timeline:**
1. Taker signs order offchain
2. Broadcast to SWIFT WebSocket (~50ms)
3. Maker receives order (~100-500ms total)
4. Maker submits place-and-make tx
5. Taker's order lands onchain (~1-2s total)
6. Maker's fill tx lands (ideally same slot)
...
**Timeline:**
1. Taker submits tx to Solana
2. Tx lands onchain (~1-2s)
3. RPC/gRPC subscription fires
4. Maker sees auction, computes fill
5. Maker submits place-and-make tx
6. Fill lands (~1-2s later)
