...
### 1. JIT Auction (first ~10 slots / ~5 seconds)

The taker order enters an auction where market makers compete to fill at the best price. This is the highest-priority liquidity source because it produces the best price discovery — makers actively compete in real-time. The auction price interpolates from the start price toward the end price over the auction duration, creating a Dutch auction dynamic.
...
For live orderbook feeds with sub-second updates:
...
3. **Walk the book**
   - Continue filling at successive price levels until taker is fully filled or book is exhausted
...
- **L2 `sources` is an object, not a string** — each price level's `sources` field maps source names to size strings (e.g., `{ "dlob": "5000000", "vamm": "10000000" }`). Don't try to parse it as a flat value.
