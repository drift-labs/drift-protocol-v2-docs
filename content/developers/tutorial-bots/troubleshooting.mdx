...
The account in `KEEPER_PRIVATE_KEY` has not been initialised on the `ClearingHouse` program yet. See [Keeper Bots](/developers/tutorial-bots/keeper-bots) for how to initialise the `ClearingHouseUser`
