...
Matching Bots are similar to [Tutorial: Order Trigger Bot](/developers/tutorial-bots/keeper-bots/tutorial-order-trigger-bot) in that they:
...
Follow the instructions at [Keeper Bots](/protocol/about-v3/keepers) to set the required environment variables and make sure a `ClearingHouseUser` is initialized.
...
Market orders that are sent on the Drift Protocol first go through the [Just-In-Time (JIT) Auctions](/protocol/about-v3/matching-engine). After the auction period, Matching Bots step in to fill orders for a small reward.
