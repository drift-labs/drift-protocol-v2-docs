...
The Drift SDK (`@drift-labs/sdk`) is a TypeScript library that handles onchain interactions, account subscriptions, and transaction construction. Understanding its internals helps you optimize performance, debug issues, and use advanced features.
...
const driftClient = new DriftClient({
  ...
  accountSubscription: {
    type: "polling",
    frequency: 1000,  // Poll every 1000ms
  },
});
```
...
```typescript
import { GrpcSubscription } from "@drift-labs/sdk";

const grpcConfig = {
  endpoint: "grpc.mainnet.jito.wtf",
...
// Add accounts to load
bulkAccountLoader.addAccount(userAccountPubkey, (buffer) => {
  // Callback when account updates
  const account = program.coder.accounts.decode("User", buffer);
...
This batches multiple `getMultipleAccounts` calls for efficiency.
...
const eventSubscriber = new EventSubscriber(connection, program);
await eventSubscriber.subscribe();

eventSubscriber.eventEmitter.on("OrderFillRecord", (event) => {
  console.log("Order filled:", event);
  console.log("  User:", event.user.toString());
  console.log("  Market:", event.marketIndex);
  console.log("  Fill price:", event.price.toString());
});
```

Events include:
- `OrderFillRecord` - Order executions
...
**Switch subaccounts**:
```typescript
driftClient.switchActiveUser(1);  // Switch to subaccount 1
...
- [Program Structure](./program-structure) - onchain accounts the SDK interacts with
