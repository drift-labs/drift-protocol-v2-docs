...
Drift’s v3 AMM is still a AMM using a constant product curve but now includes external [Backstop AMM Liquidity (BAL)](/backstop-amm-liquidity/bal-provider-overview) a concentration factor, and dynamic spread/peg that programmatically update prior to filling trades.
...
![](/assets/K7PAJRkd_-P-t8v6RQ_nI_drift-amm-bid-ask-spreads.png)
...
Every time a trader wants to fill against the AMM, the AMM will be updated in the same slot with a valid oracle price (see [Oracles](/protocol/trading/oracles)).
